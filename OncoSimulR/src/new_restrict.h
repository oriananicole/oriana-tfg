...
typedef std::map<std::vector<int>, Deme> MapaDemes;
...
struct SpatialOutput
{
  ...
  //summary
  std::vector<std::string> vector_Genotypes;
  std::vector<int> vector_popSize;
  std::vector<int> vector_numDemes;
  std::vector<int> vector_numDemesGanador;

  //Simulation (mapa de demes por iteracion)
  std::vector<std::vector<int>> vector_iter_numDemesGanador;
  //PopSize and Demes for iteration
  std::vector<std::vector<int>> vector_iter_popSize;
  std::vector<std::vector<int>> vector_iter_numDemes;
  size_t keepEveryIters;
};

//struct SpatialOutput_aux1 {
//  Genotype genot;
//  int popSize;
//  int numDemes;
//  int numDemesGanador;
//
//  bool operator<(SpatialOutput_aux1 &a) const{
//    return popSize > a.popSize;
//  }
//};

struct SpatialSimulation_aux1
{
...
typedef std::map<std::vector<int>, SpatialSimulation_aux1> MapaSimulacion;
