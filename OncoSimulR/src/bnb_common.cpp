...
void updateRatesConstante(std::vector<spParamsP> &popParams,
                          ...
                          const double &sampleEvery)
{
  ...
  double fitnessMedio = 0, fitness, fitnessFactor, cteFitness;
  //Se calcula el fitness maximo y la media de fitness de las células
  for (size_t i = 0; i < popParams.size(); ++i)
  {
    fitness = prodFitness(evalGenotypeFitness(genotypes[i], F, genotypes, popParams));

    //DEBUG
    //Rcpp::Rcout << "\n pop " << popParams[i].popSize << " ---- fitness: " << fitness;
    fitnessMedio += fitness * popParams[i].popSize;
  }

  if ((totPopSize > 0) && (maxFitness > 1))
  {
    fitnessMedio = fitnessMedio / totPopSize;
    fitnessFactor = (fitnessMedio - 1) / (maxFitness - 1);
    cteFitness = initSize + (cte - initSize) * fitnessFactor;
  }
  else
  {
    cteFitness = initSize;
  ...
  //DEBUG
  //Rcpp::Rcout << "\n cteFitness " << cteFitness << " --- fitnessMedio: " << fitnessMedio << " --- fitnessFactor: " << fitnessFactor ;

  double adjust_r = 0.0;
  for (size_t i = 0; i < popParams.size(); ++i)
  {
    adjust_r += (popParams[i].birth - popParams[i].death) * popParams[i].popSize;
    //DEBUG
    //Rcpp::Rcout << "\n birth " << popParams[i].birth << " --- death: " << popParams[i].death << " --- size: " << popParams[i].popSize ;
  }
  adjust_r /= totPopSize;
  //DEBUG
  //Rcpp::Rcout << "\n totPopSize " << totPopSize << " --- M: " << M;

  adjust_r = adjust_r + (totPopSize - cteFitness) / (totPopSize * (sampleEvery));
  //DEBUG
  //Rcpp::Rcout << "\n Ajuste Moran " << adjust_r << " --- tiempo: " << sampleEvery;

  for (size_t i = 0; i < popParams.size(); ++i)
  {
    //OJO QUE PUEDE SALIR DEATH < 0. En ese caso no lo permitimos:
    popParams[i].death += adjust_r;
    if (popParams[i].death < 0.1)
    {
      popParams[i].death = 0.1;
    }
    else if (popParams[i].death > maxFitness + 0.5)
    {
      popParams[i].death = maxFitness + 0.5;
    }

    //if(adjust_r > 0){
    //  popParams[i].death += adjust_r;
    //} else {
    //  popParams[i].birth -= adjust_r;
    //}
    W_f_st(popParams[i]);
    R_f_st(popParams[i]);
  }
