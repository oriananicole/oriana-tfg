...
oncoSimulIndiv <- function(fp,
                           ...
                           fixation = NULL,
                           seed = NULL
                           initMutantPercentage = 0.1,
                           cteSize = -1,
                           migrationProb = 0.5,
                           largeDistanceMigrationProb = 1e-6,
                           ...
                           spatialKeepEvery = 1,
                           spatialVerbosity = 2,
                           spatialDeleteDemesWithoutMutations = FALSE
                           ) {
    call <- match.call()
    ...
    if((spatialModel == FALSE)&&(typeFitness=="constante")){
        stop("Using Constante out of spatial models is not implemented yet")
    }
    ...
    if(spatialModel == TRUE){
        ...
        if((spatialIterMax <= 0) && (spatialDemesMax <= 0)){
            stop("There is not stopping conditions for spatial model")
        }
        if(typeFitness != "constante"){
            warning("No good results are guaranteed for the spatial model with typeFitness = constante")
        }
        if(cteSize < initSize){
            stop("cteSize must >= initSize")
        }
    }    
    ...
    if(spatialModel){
        ...
        if((op$Simulation$KeepEveryIters == 1) && (op$TotalPopulation == 0)){
            ...
            file <- 'OncoSimulR.spatial'
            op$Simulation$Data <- read.table(paste0(file))
            file.remove(file)   
        }
    }
