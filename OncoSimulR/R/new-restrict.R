...
nr_oncoSimul.internal <- function(rFE,
    ...
    return(c(
        nr_BNB_Algo5(rFE = rFE,
                     ...
                     largeDistanceMigrationProb = largeDistanceMigrationProb,
                     maxMigrationPercentage = maxMigrationPercentage,
                     spatialModel = spatialModel,
                     spatialDemesMax = spatialDemesMax,
                     spatialIterMax = spatialIterMax,
                     spatialKeepEvery = spatialKeepEvery,
                     spatialVerbosity = spatialVerbosity,
                     spatialDeleteDemesWithoutMutations = spatialDeleteDemesWithoutMutations),
...
simulation3D <- function(ep2, manual = FALSE) {
  if(is.null(ep2$Simulation$Data)){
    stop("There is not a possible simulation")
  }
  dev.new()
  a = dev.cur()
  for(i in 0:(ep2$Simulation$KeepEveryIters-1)){
    dev.set(a)
    par(mfrow = c(2,1), mar=c(4,4,4,4)) 
    barplot(ep2$Simulation$WinDemesPerIter[[i+1]],
            main = paste0("Demes number (", as.character(i), ")"),
            col = 1:length(ep2$Simulation$Genotypes))
    barplot(ep2$Simulation$PopSizePerIter[[i+1]],
            main = paste0("Population size (", as.character(i), ")"),
            col = 1:length(ep2$Simulation$Genotypes))
    columna <- paste0("e_", as.character(i))
    columna2 <- paste0("g_", as.character(i))
    size <- as.numeric(cut(ep2$Simulation$Data[[columna]],c(0,1)))
    genotList <- split(ep2$Simulation$Data, size)
    n = length(genotList)
    if(n>0){
      plot3d( genotList[[n]]$X, 
              genotList[[n]]$Y, 
              genotList[[n]]$Z, 
              xlab = 'x',
              ylab = 'y',
              zlab = 'z',
              main = paste0("3D simulation growth (", as.character(i), ")"),
              col = genotList[[n]][[columna2]], 
              type = "s", 
              radius = 0.2 )
      legend3d("topright", 
               legend = ep2$Simulation$Genotypes,
               pch = 16, 
               col = 1:length(ep2$Simulation$Genotypes), 
               cex=1, 
               inset=c(0.02))
